...
class BattleFirstPokemonListView extends ListView {
  BattleFirstPokemonListView(
    ...
    List<Color> opponentColorFilters, {
    required bool isInput,
    required void Function(int) onTapOwnPokemon,
                  ...
                  for (int i = 0; i < 6; i++)
                    Row(
                      children: [
                        Expanded(
                          flex: 5,
                          child: battle.getParty(PlayerType.me).pokemons[i] !=
                                  null
                              ? Badge(
                                  smallSize: 0,
                                  offset: Offset(-10, 0),
                                  label: isInput
                                      ? checkedPokemons.own.indexWhere(
                                                  (e) => e == i + 1) >=
                                              0
                                          ? Text(
                                              '${checkedPokemons.own.indexWhere((e) => e == i + 1) + 1}')
                                          : null
                                      : ownPokemonStates![i].battlingNum != 0
                                          ? Text(
                                              '${ownPokemonStates[i].battlingNum}')
                                          : null,
                                  child: PokemonMiniTile(
                                    ...
                                    onTap: isInput
                                        ? () {
                                            if (checkedPokemons.own
                                                .contains(i + 1)) {
                                              checkedPokemons.own.removeWhere(
                                                  (e) => e == i + 1);
                                            } else if (checkedPokemons
                                                    .own.length <
                                                3) {
                                        ...
                                        : null,
                                    selected: isInput
                                        ? checkedPokemons.own.contains(i + 1)
                                        : ownPokemonStates![i].battlingNum != 0,
                              ...
                              ? Stack(
                                  children: [
                                    PokemonMiniTile(
                                      ...
                                      onTap: isInput
                                          ? () {
                                              checkedPokemons.opponent = i + 1;
                                              setState();
                                            }
                                          : null,
                                      tileColor: Color.alphaBlend(
                                          opponentColorFilters[i],
                                          theme.listTileTheme.tileColor ??
                                              Colors.white),
                                      selected: isInput
                                          ? checkedPokemons.opponent == i + 1
                                          : opponentPokemonIndex == i + 1,
