...
class ViewBattlePageState extends State<ViewBattlePage> {
  ...
  Widget build(BuildContext context) {
    ...
    switch (pageType) {
      case RegisterBattlePageType.basePage:
        title =
            Text('${loc.battlesTabTitleBattleBase} - ${widget.battle.name}');
        lists = BattleBasicListView(
          ...
          ownPartyController,
          () {
            return Future<Party?>.value(null);
          },
      ...
      case RegisterBattlePageType.firstPokemonPage:
        title = Text(
            '${loc.battlesTabTitleSelectingPokemon} - ${widget.battle.name}');
      ...
      case RegisterBattlePageType.turnPage:
        title =
            Text('${loc.battlesTabTitleTurn}$turnNum - ${widget.battle.name}');
        lists = Column(
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Row(
                    children: [
                      ...
                      Flexible(
                          child: Text(
                        _focusingPokemon(PlayerType.me, focusState!).name,
                ...
                Expanded(
                  child: Row(
                    children: [
                      ...
                      Flexible(
                          child: Text(
                        _focusingPokemon(PlayerType.opponent, focusState).name,
                        overflow: TextOverflow.ellipsis,
                      )),
                      focusState
                          ...
                          children: [
                                  ...
                                  child: Row(children: [
                                    viewMode == 0
                                        ...
                                        : viewMode == 1
                                            ...
                                            : viewMode == 2
                                                ? Text(loc
                                                    .battlesTabStatusModeStatusNoCorrection)
                                                : Text(loc
                                                    .battlesTabStatusModeStatusWithCorrection),
                                    SizedBox(width: 10),
                                    Icon(Icons.sync),
                                  ]),
                                ),
                            ...
                            _HPBarRow(
                                focusState
                                    .getPokemonState(PlayerType.me, null)
                                    .remainHP,
                                _focusingPokemon(PlayerType.me, focusState)
                                    .h
                                    .real,
                                focusState
                                    .getPokemonState(PlayerType.opponent, null)
                                      ...
                                      i < 6
                                          ? _StatStatusViewRow(
                                              ...
                                              focusState
                                                  .getPokemonState(
                                                      PlayerType.me, null)
                                                  .minStats[StatIndexNumber
                                                      .getStatIndexFromIndex(i)]
                                                  .race,
                                              focusState
                                                  .getPokemonState(
                                                      PlayerType.me, null)
                                                  .maxStats[StatIndexNumber
                                                      .getStatIndexFromIndex(i)]
                                                  .race,
                                              focusState
                                                  .getPokemonState(
                            ...
                            for (int i = 0;
                                i <
                                    max(
                                        focusState
                                            .getIndiFields(PlayerType.me)
                                            .length,
                                        focusState
                                            .getIndiFields(PlayerType.opponent)
                                            .length);
                                i++)
            ...
            Expanded(
              ...
              child: BattleTurnListView(
                ...
                ownParty.pokemons[
                    turns[turnNum - 1].getInitialPokemonIndex(PlayerType.me) -
                        1]!,
                opponentParty.pokemons[turns[turnNum - 1]
                        .getInitialPokemonIndex(PlayerType.opponent) -
                    1]!,
        ...
        actions: [
          ...
          MyIconButton(
            ...
            onPressed: () => widget.onEdit(
                widget.battle, RegisterBattlePageType.basePage, 1),
  ...
  Pokemon _focusingPokemon(PlayerType player, PhaseState focusState) {
    return widget.battle
        .getParty(player)
        .pokemons[focusState.getPokemonIndex(player, null) - 1]!;
  }

  List<List<TurnEffectAndStateAndGuide>> _createSameTimingList(
    MyAppState appState,
    ...
    for (int i = 0; i < turnEffectAndStateAndGuides.length; i++) {
      ...
      if (turnEffectAndStateAndGuides[i].turnEffect.timing != timing ||
          ...
          turnEffectAndStateAndGuides[i].turnEffect.timing ==
              Timing.changeFaintingPokemon) {
        if (i != 0) {
          turnEffectAndStateAndGuides[beginIdx].updateEffectCandidates(
              currentTurn,
              turnEffectAndStateAndGuides[
                      beginIdx == 0 ? beginIdx : beginIdx - 1]
                  .phaseState);
    ...
    if (phases.isNotEmpty) {
      turnEffectAndStateAndGuides[beginIdx].updateEffectCandidates(
          currentTurn,
          turnEffectAndStateAndGuides[beginIdx == 0 ? beginIdx : beginIdx - 1]
              .phaseState);
    ...
    required AppLocalizations loc,
          ...
          children: [
            ...
            ownState.moves.length > idx && ownState.usedPPs.length > idx
                ? Text(ownState.usedPPs[idx].toString())
                : Text(''),
            SizedBox(
              width: 10,
            ),
            ...
            opponentState.moves.length > idx &&
                    opponentState.usedPPs.length > idx
                ? Text(opponentState.usedPPs[idx].toString())
                : Text(''),
            SizedBox(
              width: 10,
            ),
                ...
                children: [
                  ...
                  ClipRRect(
                    ...
                    child: Container(
                      ...
                      height: 20,
                      color: (ownRemainHP / ownMaxHP) <= 0.25
                          ? Colors.red
                          : (ownRemainHP / ownMaxHP) <= 0.5
                              ? Colors.yellow
                              : Colors.lightGreen,
                    ),
                  ),
                ...
                children: [
                  ...
                  ClipRRect(
                    ...
                    child: Container(
                      ...
                      height: 20,
                      color: opponentRemainHPPercent <= 25
                          ? Colors.red
                          : opponentRemainHPPercent <= 50
                              ? Colors.yellow
                              : Colors.lightGreen,
                    ),
                  ),
            ...
            Expanded(
              child: Row(
                children: [
                  ClipRRect(
                    borderRadius: BorderRadius.all(Radius.circular(5.0)),
                    child: ownPokemonState.ailmentsLength > index
                        ? Container(
                            color: ownPokemonState.ailments(index).bgColor,
                            child: Text(
                                ownPokemonState.ailments(index).displayName,
            ...
            Expanded(
              child: Row(
                children: [
                  ClipRRect(
                    borderRadius: BorderRadius.all(Radius.circular(5.0)),
                    child: opponentPokemonState.ailmentsLength > index
                        ? Container(
            ...
            Expanded(
              child: Row(
                children: [
                  ClipRRect(
                    borderRadius: BorderRadius.all(Radius.circular(5.0)),
                    child: state.getIndiFields(PlayerType.me).length > index
                        ? Container(
                            color: state
                                .getIndiFields(PlayerType.me)[index]
                                .bgColor,
                            child: Text(
                                state
                                    .getIndiFields(PlayerType.me)[index]
            ...
            Expanded(
              child: Row(
                children: [
                  ClipRRect(
                    borderRadius: BorderRadius.all(Radius.circular(5.0)),
                    child: state.getIndiFields(PlayerType.opponent).length >
                            index
                        ? Container(
                            color: state
                                .getIndiFields(PlayerType.opponent)[index]
                                .bgColor,
                            child: Text(
                                state
                                    .getIndiFields(PlayerType.opponent)[index]
                ...
                ? Flexible(
                    child: ClipRRect(
                      ...
                      child: Container(
                        ...
                        child: Text(state.weather.displayName,
                            style: TextStyle(color: Colors.white)),
                      ),
                    ),
                  )
                ...
                ? Flexible(
                    child: ClipRRect(
                      ...
                      child: Container(
                        ...
                        child: Text(state.field.displayName,
                            style: TextStyle(color: Colors.white)),
                      ),
                    ),
                  )
