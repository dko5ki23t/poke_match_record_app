...
extension PokeTypeEffectiveness on PokeType {
  ...
  static double effectivenessRate(
      ...
      for (final type in types) {
        switch (attackType) {
          ...
          case PokeType.fight:
            ...
            if (type == PokeType.poison ||
                type == PokeType.fly ||
                type == PokeType.psychic ||
                type == PokeType.bug ||
                type == PokeType.fairy) deg--;
            if (!isRingTarget &&
                !canNormalFightToGhost &&
                type == PokeType.ghost) return 0;
            break;
          case PokeType.fly:
            if (type == PokeType.fight ||
                type == PokeType.grass ||
                type == PokeType.bug) deg++;
          ...
          case PokeType.fire:
            ...
            if (type == PokeType.fire ||
                type == PokeType.water ||
                type == PokeType.rock ||
                type == PokeType.dragon) deg--;
            break;
          case PokeType.water:
            if (type == PokeType.fire ||
                type == PokeType.ground ||
                type == PokeType.rock) deg++;
    ...
    if (!state.isTerastaling &&
        attackType == PokeType.fire &&
        state.ailmentsWhere((e) => e.id == Ailment.tarShot).isNotEmpty) deg++;
    if (deg == 3) {
      return 8;
    } else if (deg == 2) {
      return 4;
